...
from deep_morpho.models.bise_base import (
    ClosestSelemEnum,
    BiseBiasOptimEnum,
    BiseWeightsOptimEnum,
)
...
from deep_morpho.loss import (
    ...
    MaskedDiceLoss,
    MaskedBCELoss,
    QuadraticBoundRegularization,
    LinearBoundRegularization,
...
class ArgsEnforcersCurrent(ArgsEnforcer):
    def add_enforcer(self):
        def enforce_fn(experiment):
            ...
            # if experiment.args["dataset"] == "noistidataset":


            if (
                isinstance(experiment.args["loss_regu"], tuple)
                and experiment.args["loss_regu"][0] == "RegularizationProjActivated"
            ):
                experiment.args["closest_selem_method"] = ClosestSelemEnum.MIN_DIST_ACTIVATED_POSITIVE

            elif (
                isinstance(experiment.args["loss_regu"], tuple)
                and experiment.args["loss_regu"][0] == "RegularizationProjConstant"
            ):
            ...
            if "loss_regu_delay" in experiment.args and experiment.args["loss_regu_delay"] > 0:
            ...
            experiment.args["patience_reduce_lr"] = max(
                int(experiment.args["patience_loss"] * experiment.args["patience_reduce_lr"]) - 1, 1
            )
            ...
            if isinstance(experiment.args["initializer_args"], dict):
                ...
                experiment.args["init_bise_str"] = [
                    str(ar["bise_init_method"]) for ar in experiment.args["initializer_args"]
                ]

            if experiment.args["atomic_element"] == "sybisel":
                experiment.args["threshold_mode"] = {
                    "weight": experiment.args["threshold_mode"]["weight"],
                    "activation": experiment.args["threshold_mode"]["activation"] + "_symetric",
                }
                experiment.args["bias_optim_mode"] = BiseBiasOptimEnum.RAW
            ...
            experiment.args["loss_data"] = loss_dict[experiment.args["loss_data_str"]](
                **experiment.args["kwargs_loss_data"]
            )
            ...
            if experiment.args["loss_regu"] != "None":
                ...
                experiment.args["loss_regu"] = loss_dict[experiment.args["loss_regu_str"]](
                    **experiment.args["kwargs_loss_regu"]
                )
